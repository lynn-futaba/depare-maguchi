...
class DepalletAreaRepository(IDepalletAreaRepository):
    ...
    #         kanban_map = {
    #             1: 2001, # Bライン (button_id 1～6 R1,R2,R3,L1,L2,L3)
    #             2: 2002, 
    #             3: 2003, 
    #             4: 2004, 
    #             5: 2005, 
    #             6: 2006, 
    #             7: 1001, # Aライン (button_id 7～12 R1,R2,R3,L1,L2,L3)
    #             8: 1002, 
    #             9: 1003, 
    #             10: 1004, 
    #             11: 1005, 
    ...
    #         elif line_frontage_id in (6, 12): # ( Bライン=> L3 button_id 6, Aライン=> L3 button_id 12)
    #             signal_ids = (8231, 8216) # ( Bライン/ Aライン, L3 => 4,3)
    ...
    #         #     # Bライン, 間口 5,4,3,2,1
    #         #     1: (8061, 8047, 8032, 8017, 8002), # R1 => button_id 1
    #         #     2: (8061, 8047, 8032, 8017, 8002), # R2 => button_id 2
    #         #     3: (8061, 8047, 8032),             # R3 => button_id 3
    ...
    #         # add sugiura
    #         ids_step4 = signal_map.get("ni_herasu", {}).get(line_frontage_id, [])

    #         # add sugiura  *ids_step4
    #         all_ids = [*ids_step1, *ids_step2, *ids_step3, *ids_step4]
    ...
    #         # add sugiura
    #         if ids_step4:
    #             conditions.append(f"WHEN signal_id IN ({','.join(['%s']*len(ids_step4))}) THEN 1")
    #             params.extend(ids_step4)
    ...
    #         # Check Step 4 signals
    #         if ids_step4:
    #             placeholders4 = ','.join(['%s'] * len(ids_step4))
    #             cur.execute(f"SELECT COUNT(*) FROM eip_signal.word_input WHERE signal_id IN ({placeholders4}) AND value = 1", ids_step4)
    #             count = cur.fetchone()[0]
    #             logging.info(f"[DepalletAreaRepository >> call_AMR_return() >> ✅ Step 2 active signals]: {count}")

    #             # Reset Step 2 to 0
    #             cur.execute(f"UPDATE eip_signal.word_input SET value = 0 WHERE signal_id IN ({placeholders4})", ids_step4)
    #             logging.info(f"[DepalletAreaRepository >> call_AMR_return() >> ✅ Step 2 reset to 0]")
    #             conn.commit()
    ...
    def call_AMR_return(self, line_frontage_id):
        ...
        signal_map = {
            "hashiru_ichi": { # ①一a # 呼び出し信号をリセット / デパレ間口()実TP の 呼出
                # Bライン
                1: (8062, 8046, 8031, 8016, 8000), # R1 間口 5,4,3,2,1 => button_id 1
                2: (8062, 8046, 8031, 8016, 8000), # R2 間口 5,4,3,2,1 => button_id 2
                ...
                # Aライン
                7: (8062, 8046, 8031, 8016, 8000), # R1 間口 5,4,3,2,1 => button_id 7
                8: (8062, 8046, 8031, 8016, 8000), # R2 間口 5,4,3,2,1 => button_id 8
            ...
            "hashiru_ni": { # ③三 # 搬送指示 間口からストアに搬送 / デパレ間口()発進
                # Bライン
                1: (8061, 8047, 8032, 8017, 8002), # R1 間口 5,4,3,2,1 => button_id 1
                2: (8061, 8047, 8032, 8017, 8002), # R2 間口 5,4,3,2,1 => button_id 2
                3: (8061, 8047, 8032),             # R3 間口 5,4,3 => button_id 3
                ...
                # Aライン
                7: (8061, 8047, 8032, 8017, 8002), # R1 間口 5,4,3,2,1 => button_id 7
                8: (8061, 8047, 8032, 8017, 8002), # R2 間口 5,4,3,2,1 => button_id 8
                9: (8061, 8047, 8032),             # R3 間口 5,4,3 => button_id 9
