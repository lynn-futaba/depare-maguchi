...
class LineRepository( ILineRepository):
    ...
    def get_lines(self,line_id_list:list)->list[Line]:
        # lines = [] # TODO: added this line
        try:
            # placeholders = ', '.join(['%s'] * len(line_id_list)) # TODO: added this line
            # sql =f"SELECT * FROM depal.line where line_id in {placeholders};" # TODO: added this line
            sql =f"SELECT * FROM depal.line where line_id in {line_id_list};"
            conn =self.db.depal_pool.get_connection()
            cur = conn.cursor(dictionary=True)
            cur.execute(sql)
            # cur.execute(sql, line_id_list)  # TODO: added this line , pass parameters safely
            result= cur.fetchall()
            ...
            for line in lines:
                ...
                result= cur.fetchall()
                # sql_frontage = "SELECT * FROM depal.line_frontage WHERE line_id = %s;" # TODO: added this line , pass parameters safely
                # cur.execute(sql_frontage, (line.id,)) # TODO: added this line , pass parameters safely
                # result = cur.fetchall() # TODO: added this line , pass parameters safely
                ...
                for frontage in line.frontages.values():
                    ...
                    # sql_inventory  = f"SELECT * FROM depal.line_inventory inner join depal.m_product using(part_number) where frontage_id = %s;" # TODO: added this line
                    # cur.execute(sql)
                    cur.execute(sql, (frontage.id,))
                    inv = cur.fetchall()
                    inventories = []
                    for row in inv:
                        inventory = Inventory(row["inventory_id"],Part(row["part_number"],row["kanban_no"],
                                                                       ["name"],row["car_model_id"]), row["case_quantity"])
    ...
    def  supply_parts(self,flow_rack:FlowRack):
        ...
        try:
            sql = "INSERT INTO depal.supply (inventory_id,quantity) VALUES (%s,%s);"
            conn = self.db.depal_pool.get_connection()
            cur = conn.cursor()
